...

{
  ...
  echo "JAVA_OPTS=\"-Dapp.home=$APP_HOME -Dwebfe.tuxedo.client.mode=$WEBFE_TUXEDO_CLIENT_MODE -Dwebfe.tuxedo.jolt.listen=$WEBFE_TUXEDO_JOLT_LISTEN -DTM_ALLOW_NOTLS=$TM_ALLOW_NOTLS\""
  echo "# CNAPS POC end"
} >> "$tmp"

sudo install -m 0644 "$tmp" "$TOMCAT_SYSCONFIG"
rm -f "$tmp"
echo "Configured $TOMCAT_SYSCONFIG for APP_HOME=$APP_HOME and mode=$WEBFE_TUXEDO_CLIENT_MODE"
